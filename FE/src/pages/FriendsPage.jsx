...
function FriendsPage() {
  ...
  const {refreshCounts, updateCount} = useNotification();
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const list = data || [];             
      setPending(list);
      updateCount('campRequest', list.length);  
  
    } catch (err) {
      console.error('fetchPendingRequests error', err);
      setPending([]);
      updateCount('campRequest', 0);
    }
  };
  ...
  const handleAccept = async (requesterNickname) => {
    ...
    try {
      ...
      refreshCounts();//수락했으니 알림배지도 즉시갱신
    } catch (err) {
  ...
  const handleReject = async (requesterNickname) => {
    ...
    try {
      ...
      // 성공 시 로컬 목록에서도 제거
      setPending((prev) => { 
        const next = prev.filter((req) => req.nickname !== requesterNickname);
        updateCount('campRequest', next.length); //  배지도 함께 갱신
        return next;
      });
    } catch (e) {
